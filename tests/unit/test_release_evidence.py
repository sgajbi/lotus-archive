...
def test_release_evidence_binds_image_digest_to_runtime_metadata() -> None:
    ...
    assert image["runtime_environment"]["LOTUS_ARCHIVE_IMAGE_DIGEST"] == "sha256:" + "b" * 64
    assert image["runtime_environment"]["LOTUS_ARCHIVE_IMAGE_REF"] == (
        "ghcr.io/sgajbi/lotus-archive:" + "a" * 40
    )
    assert image["oci_labels"]["org.opencontainers.image.revision"] == "a" * 40
    assert image["oci_labels"]["io.lotus.image.digest"] == "sha256:" + "b" * 64
    assert evidence["sbom"]["image_bound"] is True
    assert evidence["provenance"]["image_bound"] is True
